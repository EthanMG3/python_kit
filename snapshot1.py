...
def read(fname,kind=0): 
  ...
  try:  
    ...
    sect6=mpsi*nvgrid*2*nspecies
    sect7=2*nvgrid*tgrid*nvgrid*nvgrid*nspecies
    ...
    pdf2dpsidataseries=np.zeros(sect6,dtype=float)
    fullpdfseries=np.zeros(sect7,dtype=float)
    ...
    pdf2dpsidata=np.zeros((mpsi,nvgrid,2,nspecies),dtype=float)
    
    fullpdfdata=np.zeros((nvgrid,tgrid,nvgrid,nvgrid,2,nspecies),dtype=float)

    ## this separates the files into the four sections as a series of float numbers
    if kind==0 or kind==1 or kind==6:
        for i in range(0,sect1):
            profileseries[i]=float(lines[7+i])
    if kind==0 or kind==2 or kind==6:
        for i in range(0,sect2): 
            pdfseries[i]=float(lines[7+sect1+i])
    if kind==0 or kind==3 or kind==6:
    ...
    if kind==0 or kind==5 or kind==6:
        for i in range(0,sect5): 
            pdf2ddataseries[i]=float(lines[7+sect1+sect2+sect3+sect4+i])
    if  kind==6 or kind==6:
        for i in range(0,sect6): 
            pdf2dpsidataseries[i]=float(lines[7+sect1+sect2+sect3+sect4+sect5+i])
    if kind==0 or kind==7:
        for i in range(0,sect7):
            fullpdfseries[i]=lines[7+sect1+sect2+sect3+sect4+sect5+sect6+i]
    ...
    if kind==0 or kind==6:    
        for i in range(0,nspecies):
            for j in range(0,2):
                for k in range(0,nvgrid):
                    for l in range(0,mpsi):
                        pdf2dpsidata[l,k,j,i]=pdf2dpsidataseries[l+k*mpsi+(j*mpsi*nvgrid)+(i*mpsi*nvgrid*2)]
    
    if kind==0:
        for n in range(0,nspecies):
            for i in range(0,2):
                for j in range(0,nvgrid):
                    for k in range(0,nvgrid):
                        for l in range(0,tgrid):
                            for m in range(0,nvgrid):
                                fullpdfdata[m,l,k,j,i,n]=fullpdfseries[
                                    m+l*nvgrid+k*nvgrid*tgrid+j*tgrid*nvgrid**2+i*tgrid*nvgrid**3+n*2*tgrid*nvgrid**3]
        print('third ',m+(l*nvgrid)+(k*nvgrid*tgrid)+(j*tgrid*nvgrid**2)+(i*tgrid*nvgrid**3)+(n*2*tgrid*nvgrid**3))                     
                    
    if kind==0:
        return (profile, pdf, poloidata,fluxdata,pdf2ddata,pdf2dpsidata,fullpdfdata,nspecies,nfield,nvgrid,mpsi,mtgrid,mtoroidal,tmax)
    elif kind==1:
        return (profile,nspecies,nfield,nvgrid,mpsi,mtgrid,mtoroidal,tmax)
    elif kind==2:
        return (pdf,nspecies,nfield,nvgrid,mpsi,mtgrid,mtoroidal,tmax)
        ...
        return (profile, pdf, poloidata,fluxdata,pdf2ddata,pdf2dpsidata,nspecies,nfield,nvgrid,mpsi,mtgrid,mtoroidal,tmax)
  except:
...
def fluxshow(fluxdata, kind = 0, savefig = 0):
    ...
    if kind==0:
        ...
        ax.set(xlabel='toroid/pdfal', ylabel='tgrid',title='dene')
        fig.colorbar(figdene)
